  ...
  <div class = "question_details">'.$q->details().'</div>
    ...
    if($appendTags)
    {
        ...
        foreach($tags as $k => $v)
        {
            ...
            {
                $ret .= '<li class = "tag"><a href = "'.$sTemplate->getRoot().$v.'/">'.$v.'</a></li>';
            }
        }
    }
...
<div class = "question">
  <div class = "question_title">
    <a href = "'.$q->url().'">
      <p>
        <span style = "vertical-align: middle;">'.$q->title().'</span>
      </p>
    </a>
  </div>
</div>';
...
<div class = 'vote_distribution'>";
  ...
  <form action = '".$q->url()."' method = 'POST' id = 'faction_checkin_pro' onsubmit = '".$onSubmit."'>
    ...
    <button class = 'checkin_pro' ></button>
  </form>";
    }else
    {
        $content .= "
  ...
  <div class = 'distribution'>";
    if($q->percPro() == 1)
    {
        $content .= "<div class = 'distribution_pro' style = 'width: ".ceil($q->percPro() * 408)."px;'></div>
    <div class = 'distribution_con ' style = 'width: ".(408 - ceil($q->percPro() * 408 + 7))."px; border-radius:".computeBorderRadius(408 - ceil($q->percPro() * 408))."'></div>";
    }else if($q->percCon() == 1)
    {
        $content .= "<div class = 'distribution_pro' style = 'width: ".ceil($q->percPro() * 408)."px; border-left: none;'></div>
    <div class = 'distribution_con ' style = 'width: ".(408 - ceil($q->percPro() * 408))."px; border-radius:".computeBorderRadius(408 - ceil($q->percPro() * 408))."'></div>";
    }else
    {
        $content .= "<div class = 'distribution_pro' style = 'width: ".ceil($q->percPro() * 408 + 7)."px'></div>
    <div class = 'distribution_con ' style = 'width: ".(408 - ceil($q->percPro() * 408 + 7))."px; border-radius:".computeBorderRadius(408 - ceil($q->percPro() * 408))."'></div>";
    }
    $content .= "</div>";
  ...
  <form action = '".$q->url()."' method = 'POST' id = 'faction_checkin_con' onsubmit = '".$onSubmit."'>
    ...
    <input type = 'hidden' name = 'faction_select' value = '1' />
    <button class = 'checkin_con' ></button>
  </form>";
    }else
    {
        $content .= "
...
<script>wikiargument.updateDistributionBar(".($q->percPro() * 100).", ".($q->percCon() * 100).");</script>
    ";
    echo $content;
}
...
{
    $borderRadius = "";
    if($argumentConWidth > 393)
    {
        $borderRadius .= (15 - (408 - $argumentConWidth))."px "."15px 15px ".(15 - (408 - $argumentConWidth))."px";
    }else
    {
...
<div class = "argument_full">
  <div class = "stats question_stats argument_full_stats">
    <div class = "points question_points">'.$numPoints.'</div>
    <div class = "vote_line"></div>
    <div class = "votes">
      '.voteUp('question_vote_up', $q->questionId(), $argumentId, $a->type(), $q->type(), $q->flags()).'
